...
# ====================================================================================================
# 配置路径
data_dir = r"E:\代码库 数据库\混合数据集\1个时间段数据集"
...
for user_folder in user_folders:
    ...
    # 为用户生成唯一ID
    if user_folder not in user_ids:
        user_id = str(uuid.uuid4())
        user_ids[user_folder] = user_id
    else:
        user_id = user_ids[user_folder]
    ...
    for posture in postures:
        try:
            # 查找触摸数据和传感器数据文件
            try:
                touch_file_path = find_file_name(user_folder, f'*_pattern_lock_{posture}_touchData*')
                sensor_file_path = find_file_name(user_folder, f'*_pattern_lock_{posture}_sensorData*')
                ...
                print(f"用户 {user_folder} {posture} 姿势的文件未找到: {e}")
                continue
            except Exception as e:
                print(f"读取用户 {user_folder} {posture} 姿势的数据时出错: {e}")
                continue
            ...
            print(f"处理用户 {user_folder} {posture} 姿势时出现未预期的错误: {e}")
            continue

# 保存数据
print(f"\n总共处理了 {len(filtered_touch_data)} 行触摸数据")
print(f"总共处理了 {len(filtered_sensor_data)} 行传感器数据")

filtered_sensor_data.to_csv(output_sensor_path, index=False)
filtered_touch_data.to_csv(output_touch_path, index=False)
print(f"\n数据已保存到:")
print(f"  传感器数据: {output_sensor_path}")
print(f"  触摸数据: {output_touch_path}")
print("所有用户处理完成！")